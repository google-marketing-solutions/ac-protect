...
"""Tests for the App Store Collector."""
...
def _create_mock_appstore_response(
  content: dict[str, Any], success: bool = True
) -> Response:
...
class TestAppStoreCollector:
  """Tests for the AppStoreCollector class.

  Tests that the AppStoreCollector correctly:
  - Initializes with expected parameters
  - Collects app data from the App Store API
  - Processes collected data into expected format
  - Looks up individual apps in the App Store
  """
  ...
  def test_collect_returns_df_with_correct_columns(
    self, mock_get, collector, content
  ):
  ...
  def test_process_returns_df_with_correct_columns(self, collector, content):
    ...
    assert isinstance(res, pd.DataFrame)
    assert set([field.name for field in fields(tables.AppStoreTable)]).issubset(
      res.columns
    )

  @mock.patch('server.collectors.app_store.requests.get')
  def test_lookup_app_in_appstore_returns_correct_columns(
    self, mock_get, collector, content
  ):
    mock_get.return_value = _create_mock_appstore_response(content)

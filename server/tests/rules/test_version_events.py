...
"""Tests for the VersionEventsRule class.

The VersionEventsRule checks if certain events occur between versions for
specified apps. These tests verify the rule's functionality for detecting
missing events and generating appropriate alerts.
"""
...
def fixture_fake_collector_data(
  app_store_df, collector_ga4, gads_df, play_store_df
):
  return {
...
class TestVersionEventsRule:
  """Test class for VersionEventsRule.

  Tests the functionality of the VersionEventsRule class which monitors events
  between app version updates. Verifies initialization, rule checking, alert
  creation, version comparison, and data collection functionality.
  """
  ...
  def test_get_data(
    ...
    mock_get_values.side_effect = [
      fake_collector_data['app_store'],
      fake_collector_data['ga4'],
      fake_collector_data['gads'],
      fake_collector_data['play_store'],
    ]
  ...
  def test_check_rule_no_ga4_data_logs_error(
    self, mock_logger, version_events_obj, fake_collector_data
  ):
  ...
  @mock.patch('server.rules.version_events.logger')
  def test_check_rule_no_gads_data_logs_error(
    self, mock_logger, version_events_obj, fake_collector_data
  ):
    data = fake_collector_data.copy()
    del data['gads']
  ...
  def test_check_rule_only_app_store_data(
    self, version_events_obj, fake_collector_data
  ):
    num_violations = 4
    num_store_violations = 2
    ...
    assert len(store_violations) == num_store_violations
    assert all(
      isinstance(item, version_events.VersionEventsEvent) for item in violations
    )
    assert all(event.app_id.isnumeric() for event in store_violations)

  def test_check_rule_only_play_data_less_than_24_hours(
    self, version_events_obj, fake_collector_data
  ):
    num_violations = 4
    num_store_violations = 0
    collectors = fake_collector_data.copy()
  ...
  def test_check_rule_only_play_data__more_than_24_hours(
    self, version_events_obj, fake_collector_data
  ):
    num_violations = 4
    num_store_violations = 1
    collectors = fake_collector_data.copy()
    del collectors['app_store']
  ...
  def test_check_rule_no_app_store_and_play_data(
    self, version_events_obj, fake_collector_data
  ):
    num_violations = 4
    num_store_violations = 0
  ...
  def test_create_alerts(
    self,
    version_events_obj,
    fake_version_event_1,
    fake_version_event_2,
    fake_version_alert,
  ):
    num_alerts = 2
    ...
    assert len(event_alerts) == num_alerts
    assert all(isinstance(a, alerts.Alert) for a in event_alerts)
    assert event_alerts[0] == fake_version_alert

  def test_update_alerts_log__pass(
    self, version_events_obj, fake_version_alert
  ):
  ...
  def test_add_app_ids__added_app_id_column(
    self, version_events_obj, gads_df, collector_ga4
  ):

...
class PlayStoreCollector(Collector):
  ...
  def __init__(self,  auth: dict[str,str], apps: list[str], bq: BigQuery
               ) -> None:
  ...
  def save(self, df: pd.DataFrame, overwrite: bool = False) -> None:
    ...
    Args:
      ...
      False.
    """
    logger.info('Play Store Collector - saving data to %s',
  ...
  def create_play_credentials(self, auth: dict[str,str]
  ...
  def get_app_from_play_store(self, package_name: str) -> dict[str,str]:
    with discovery.build('androidpublisher', 'v3', credentials=self.creds
                         ) as service:
    ...
    return edit_response.tracks().list(
          packageName=package_name, editId=edit_id).execute()

  def extract_version_and_track(self, cell_dict: dict[str,str]

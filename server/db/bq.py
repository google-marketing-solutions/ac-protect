...
class BigQuery(BaseDb):
  ...
  def connect(self, project_id: Optional[str] = None) -> bigquery.Client:
    """Connect to BigQuery.
  ...
  def get_dataset(self, dataset_id: Optional[str]) -> bigquery.Dataset:
  ...
  def get_values_from_table(
    self, table_id: str, cols: List[str] = None, where: str = ''
  ...
  def write_to_table(
    self, table_id: str, df: pd.DataFrame, overwrite: bool = False
  ) -> bool:
  ...
  def update_last_run(self, name: str, type_: str) -> bool:
    """Updates the AC Protect log for last run.

    Updates the log with the last time the collector, rule or service ran.
  ...
  def get_alerts_for_app_since_date_time(
    self, app_id: str, date_time: datetime.datetime
  ) -> Optional[pd.DataFrame]:
    ...
    filter_ += (
      f" AND timestamp >= CAST('{date_string}' AS DATETIME FORMAT"
  ...
  def write_alerts(self, alerts: List[Alert]) -> bool:
    ...
    return self.write_to_table(
      ALERTS_TABLE_NAME, pd.DataFrame.from_dict(alerts)
    )
